...
PYTHONUNBUFFERED=1 stdbuf -oL python3 /home/a/huaqiccc_test_suite/flight/aggressive_trajectory.py --radius ${TRAJ_RADIUS:-1.0} --period ${TRAJ_PERIOD:-20.0} --rate 50 --output "huaqiccc_flatness_m${MPCA_MODE}_ff${MPCA_FF_EN}" 2>&1 | tee /tmp/flatness_flight_m${MPCA_MODE}_ff${MPCA_FF_EN}.log
