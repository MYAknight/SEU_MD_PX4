...
class TrajectoryGenerator:
    ...
    def __init__(self, radius=None, circle_period=None):
    ...
    def get_state(self, t):
        """返回: x, y, z, arm_angle, yaw, vx, vy, ax, ay, yaw_rate"""
        z = self.HOVER_Z
        omega = 2.0 * math.pi / self.CIRCLE_PERIOD
        T_R = self.T_RAMP
        ax, ay = 0.0, 0.0
        ...
        if t < self.T_HOVER0:
            ...
            if tau < T_R:
                ...
                alpha_dd = 6.0 * (1.0 - 2.0 * s) / (T_R * T_R)

                r = self.RADIUS * alpha
                r_d = self.RADIUS * alpha_d
                r_dd = self.RADIUS * alpha_dd

                x = r * math.cos(theta)
                y = r * math.sin(theta)
                vx = r_d * math.cos(theta) - r * omega * math.sin(theta)
                vy = r_d * math.sin(theta) + r * omega * math.cos(theta)
                ax = r_dd * math.cos(theta) - 2.0 * r_d * omega * math.sin(theta) - r * omega * omega * math.cos(theta)
                ay = r_dd * math.sin(theta) + 2.0 * r_d * omega * math.cos(theta) - r * omega * omega * math.sin(theta)
                ...
                ax = -r * omega * omega * math.cos(theta)
                ay = -r * omega * omega * math.sin(theta)
        ...
        return x, y, z, arm, yaw, vx, vy, ax, ay, yaw_rate
...
class UnifiedFlightTest:
    ...
    def __init__(self, enable_algo=True, rate_hz=20.0, output_prefix='huaqiccc_flight', radius=None, period=None):
        self.enable_algo = enable_algo
        self.rate_hz = rate_hz
        self.dt = 1.0 / rate_hz
        self.output_prefix = output_prefix

        self.traj_gen = TrajectoryGenerator(radius=radius, circle_period=period)
        self.records = []
    ...
    def _init_ros(self):
        ...
        try:
            ...
            # Set flatness feedforward params
            mpca_ff_en = int(os.environ.get('MPCA_FF_EN', '1'))
            mpca_ff_blend = float(os.environ.get('MPCA_FF_BLEND', '0.3'))
            mpca_ff_mass = float(os.environ.get('MPCA_FF_MASS', '1.5'))
            for p_id, p_val, p_type in [
                ('MPCA_FF_EN', mpca_ff_en, 'integer'),
                ('MPCA_FF_BLEND', mpca_ff_blend, 'real'),
                ('MPCA_FF_MASS', mpca_ff_mass, 'real'),
            ]:
                for attempt in range(3):
                    pv = ParamValue(integer=p_val) if p_type == 'integer' else ParamValue(real=p_val)
                    resp = param_set(param_id=p_id, value=pv)
                    if resp.success:
                        tuned.append(p_id)
                        print(f"[OK] Set {p_id} = {p_val}")
                        break
                    else:
                        rospy.sleep(0.5)
    ...
    def _make_position_target(self, x, y, z, yaw, vx=0.0, vy=0.0, ax=0.0, ay=0.0, yaw_rate=0.0):
        """
        PositionTarget with velocity/acceleration/yaw_rate feedforward
        """
        msg = PositionTarget()
        ...
        has_acc = abs(ax) > 1e-6 or abs(ay) > 1e-6
        has_yaw_rate = abs(yaw_rate) > 1e-6

        # Base mask: always send position + yaw
        mask = 0
        if not has_vel:
    ...
    def _send_setpoint(self, x, y, z, yaw, vx=0.0, vy=0.0, ax=0.0, ay=0.0, yaw_rate=0.0):
        self.sp_pub.publish(self._make_position_target(x, y, z, yaw, vx, vy, ax, ay, yaw_rate))
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            x, y, z, arm_angle, yaw, vx, vy, ax, ay, yaw_rate = self.traj_gen.get_state(elapsed)

            self._send_setpoint(x, y, z, yaw, vx, vy, ax, ay, yaw_rate)
            if elapsed > 18 and elapsed < 38 and int(elapsed*10) % 50 == 0:
                print(f"[SETPOINT] t={elapsed:.1f} ax={ax:.3f} ay={ay:.3f} vx={vx:.3f} vy={vy:.3f}")
...
def main():
    ...
    parser.add_argument('--period', type=float, default=20.0, help='Circle period (s)')
